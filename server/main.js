...
wsServer.on("connection", (socket) => {
    ...
    socket.on("close", (_) => {
        ...
        broadcast(TYPE_SESSION, OP_UNREADY, account);

        for (let [key, value] of connections_accounts.entries())
            if (value === account)
                connections_accounts.delete(key);
            ...
            case TYPE_AUTHENTICATION:
                ...
                switch (message[1]) {
                    case OP_REGISTER:
                        ...
                        if (exists || username.length > MAX_USERNAME_LENGTH) {
